...
# --- [شروع کدهای جدید برای قابلیت ویرایش] ---
...
if not st.session_state.logged_in:
    ...
    if is_current_archived:
        ...
        # --- [بخش جدید: دکمه و فرم ویرایش آخرین سوال] ---
        last_user_msg = None
        ...
        # نمایش دکمه ویرایش فقط اگر پیامی وجود داشته باشد و در حالت ویرایش نباشیم
        if last_user_msg and not st.session_state.editing_last:
            col_spacer, col_btn = st.columns([8, 2])
            with col_btn:
                if st.button("✏️ Edit Last Question", use_container_width=True):
                    st.session_state.editing_last = True
                    st.session_state.edit_text = last_user_msg
                    st.rerun()

        # نمایش باکس متن برای ویرایش
        if st.session_state.editing_last:
            with st.container():
                st.info("ویرایش آخرین سوال (با تایید، مکالمه قبلی حذف و جایگزین می‌شود):")
                ...
                with c1:
                    if st.button("✔️ Submit Edit", use_container_width=True):
                        # پاک کردن تعامل قبلی از دیتابیس
        ...
        # --- [بخش اصلاح شده: بررسی اینکه سوال جدید تایپ شده یا سوال قبلی در حال ویرایش است] ---
        if st.session_state.pending_prompt:
            final_prompt = st.session_state.pending_prompt
            # ریست کردن متغیر برای سوالات بعدی
            st.session_state.pending_prompt = None
