        ...
        if final_prompt:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking & Analyzing..."):
                    ...
                    # -------------------------------------------------------------
                    # 1. افزودن حافظه به موتور جستجو (Condense Question)
                    # -------------------------------------------------------------
                    search_query = final_prompt
                    chat_history_msgs = memory.chat_memory.messages
                    
                    # اگر تاریخچه‌ای وجود دارد، سوال جدید را بر اساس تاریخچه بازنویسی می‌کنیم
                    if len(chat_history_msgs) > 0:
                        chat_history_str = "\n".join([
                            f"{'User' if msg.type == 'human' else 'Assistant'}: {msg.content}" 
                            for msg in chat_history_msgs[-4:] # فقط 4 پیام آخر برای جلوگیری از هزینه اضافی
                        ])
                        
                        condense_prompt = f"""Given the following conversation and the user's new question, rephrase the new question into a standalone question containing all necessary context (like references to specific systems, parts, or conditions mentioned earlier). 
If the new question is already standalone or a simple greeting, leave it as is. Do NOT answer the question, just reformulate it for a database search.

Chat History:
{chat_history_str}

New Question: {final_prompt}
Standalone Question:"""
                        ...
                        try:
                            ...
                            # نمایش سوال بازنویسی شده به کاربر (اختیاری - برای شفافیت و دیباگ)
                            if search_query.lower() != final_prompt.lower():
                                st.caption(f"*(🔍 Contextualized Search: {search_query})*")
                                
                        except Exception as e:
                            print(f"Condense question failed: {e}")
                            # در صورت خطا، با همان سوال اصلی ادامه می‌دهیم
                            pass

                    # -------------------------------------------------------------
                    # 2. جستجو در دیتابیس با استفاده از سوال کامل شده (search_query)
                    # -------------------------------------------------------------
                    try:
                    ...
                    # -------------------------------------------------------------
                    # 3. پرامپت نهایی با قوانین سخت‌گیرانه برای توهم (Hallucination)
                    # -------------------------------------------------------------
...
Helpful Answer in English:"""
                    ...
                    # تزریق تاریخچه مکالمه به مدل نهایی
